...
boot.extend(boot.TreeService, boot.Panel, {
    ...
    _load : function(options) {
        options = options || {};
        this.parameters = options.parameters || options.data || this.parameters || {};
    ...
    _addNodes : function(nodes, parentNode) {
        for(var index in nodes){
            ...
            if(parentNode && parentNode._uuid){
                node["_checked"] = parentNode["_checked"];
    ...
    _addNodeIdentify : function(data, index) {
        ...
        node['_checked'] = node['_checked'] !== undefined ? node['_checked'] : false;
        return node;
    },
    //生成toolbar
    ...
    _makeText : function(node) {
        ...
        if (node._editting) {
            return '<span id="' + _uuid + '$text" class="tree-node-text"><input id="' + _uuid + '$editor" class="boot-textbox" value="' + text + '" height="20" style="width: auto;"/></span>';
        }
        return '<span id="' + _uuid + '$text" class="tree-node-text">' + text + '</span>';
    },
    _makeTreeData : function(list) {
        var me = this;
        var first = '', maps = {}, ListInMap = [];
    ...
    _getAttrs : function(attributes) {
        var attrs = boot.concat({
            str : ['idField', 'parentidField', 'childrenField', 'url', 'loadType', "binding", "queryMethod", "action"],
