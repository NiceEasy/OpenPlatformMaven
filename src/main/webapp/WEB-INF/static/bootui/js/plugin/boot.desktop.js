...
boot.extend(boot.DeskTop, boot.Rooter, {
    ...
    _create : function() {
        ...
        var background = jQuery('<img src="' + this.basePath + '/bootui/css/images/desktop/background.jpg" alt="" style="position: fixed;top: 0;left: 0;width: 100%;height: 100%;"/>');
    ...
    _renderShortcuts : function() {
        shorts = boot.clone(this._getShortcutsByRight(true));
        shorts = this._addActionShortcut(shorts);//新增按钮
        shorts = this._switchShortcut(shorts);//切换按钮
        shorts = this._switchOA(shorts);//OA办公
    ...
    _addActionShortcut : function(shorts) {
        var add = {
            ...
            icon : this.basePath + '/bootui/css/images/desktop/add.png'
        };
    ...
    //添加按钮
    _switchShortcut : function(shorts) {
    	var _switch = {
    			_uuid : boot.newId(),
    			_actived : false,
    			_systemIcon : 'desktop-shortcut-switch',
    			icon : this.basePath + '/bootui/css/images/desktop/add.png'
    	};
    	_switch[this.rightField] = 0;
    	_switch[this.textField] = '呼叫中心';
    	shorts.push(_switch);
  ...
  //OA按钮
    _switchOA : function(shorts) {
    	var _switch = {
    			_uuid : boot.newId(),
    			_actived : false,
    			_systemIcon : 'desktop-shortcut-OA',
    			icon : this.basePath + '/bootui/css/images/desktop/icon.png'
    	};
    	_switch[this.rightField] = 0;
    	_switch[this.textField] = 'OA办公';
    	shorts.push(_switch);
    ...
    _createPlugin : function(plugin) {
        ...
        html += '<img src="' + this.basePath + '/bootui/css/images/desktop/plugin_bg.png" alt="" style="position: absolute;top: 0;width: 100%;height: 100%;"/>';
        ...
        if (plugin.noFrame !== true) {
            ...
            html += '<iframe id="' + plugin._uuid + '$frame" ' + href + ' style="border: none;width: 100%;height: 100%;padding: 0;margin: 0;"></iframe>';
        }
        html += '</div>';
        html += '</div>';
        return html;
    ...
    _createShortcutInner : function(shortcut) {
        var icon = shortcut.icon || this.basePath + '/bootui/css/images/desktop/icon.png';
        ...
        html += '</span>';
        if (shortcut._open) {
            var open = this.basePath + '/bootui/css/images/desktop/open.png';
...
//            options.height = 550;
    ...
    _switchSystem: function(e){
    	e.stopPropagation();
    	var el = e.selector;
    	var offset = el.offset();
    	var id = boot.newId();
    	this._switchSystemID = id;
    	if(jQuery('#' + id)[0]){
    		return false;
    	}
    	var html = '<div class="system_switch" id="'+ id +'" style="left: '+ (offset.left + 15) +'px;top: '+ (offset.top - 30) +'px;display: inline-block;position: absolute;background-color: #fff;padding: 5px;border: solid 1px #999;">';
    	html += '<form style="margin: 0;" method="post" action="http://call.qskh.cn/ekt/sso.php">';
    	html += '<input type="hidden" class="user_name" name="user_name"  value="'+ this.data.user.employeeId +'"/>';
    	html += '<input onkeyup="if(this.value != \'\'){this.nextSibling.removeAttribute(\'disabled\');}else{this.nextSibling.setAttribute(\'disabled\', true);}" type="text" class="phone" name="phone" style="border: solid 1px #999;height: 22px;width: 120px;"/>';
    	html += '<input disabled="true" type="submit" value="登陆" style="border: solid 1px #999;height: 22px; margin-left: -1px;cursor: pointer;"/>';
    	html += '<span style="border: solid 8px rgba(0, 0, 0, 0);display: inline-block;border-top: solid 8px #fff;width: 0;height: 0;position: absolute;left: 16px;bottom: -15px;"></span>';
    	html += '</form>';
    	html += '</div>';
    	this.el.append(html);
    },
    
    _switchOAclick: function(e){
    	window.open(this.data.oaDomain+"/platform/console/main.ht?ticket="+this.data.user.oaticket);
    },
    ...
    _showHideShortcuts : function() {
        ...
        if (this.popupEl) {
            ...
            html += '<img src="' + this.basePath + '/bootui/css/images/desktop/hide-bg.png" alt="" style="position: absolute;top: 0;width: 100%;height: 100%;"/>';
    ...
    _bindEvents : function() {
        ...
        this._on(this.boxEl, '.desktop-shortcut-switch', 'click', this._switchSystem, this);
        this._on(this.boxEl, '.desktop-shortcut-OA', 'click', this._switchOAclick, this);
        this._on(this.el, '.system_switch', 'click', this._systemSwitch, this);
    ...
    _onBodyClick : function() {
        ...
        var _switch = jQuery('#' + this._switchSystemID);
        if(_switch[0]){
        	_switch.remove();
        }
    },
